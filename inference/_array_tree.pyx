...
Design rationale:
  ...
  (~400-500 ns per conversion, ~1250 ns total per node).  Typed
  Cython memoryviews replace all Python object creation with direct
  C pointer arithmetic (~3-5 ns per access).  Expected speedup on the
  hot path: 200-300x.  End-to-end per-flow speedup (including sklearn
  transform overhead): ~50-100x.
"""
...
def attribute_path_cy(
    ...
    """Accumulate depth-weighted attribution into feat_scores (in-place).

    feat_scores must be a C-contiguous float64 array of shape (n_features,).
    Caller zeros it before iterating over trees; this function adds to it.
    """
    cdef int node = root
    cdef int fi
    cdef int depth = 0
