        ...
        response = await client.messages.create(
            ...
            system=[{
                ...
                "text": (
                    ...
                    "All feature values in the alert are in their natural units "
                    "(packets/second, seconds, bytes, ratios 0-1, etc.) — they are NOT "
                    "IQR-normalised. Each top attribution feature carries a 'baseline' dict "
                    "with 'median' and 'iqr' in those same natural units, computed from the "
                    "baseline traffic by a RobustScaler. Use these to quantify deviation: "
                    "(value - median) / iqr gives IQR units away from typical; values beyond "
                    "±5 IQR are strong outliers. Explain the alert in 2-3 sentences, "
                    "referencing specific deviations in plain terms "
                    "(e.g. '14x the normal rate', '200 IQR above baseline'). "
                    + feature_note +
                    "Be specific about which feature values deviate and in what direction. "
                    "Only name a specific attack type (e.g. SYN flood, slowloris, SSH brute-force, "
                    "UDP flood, HTTP flood, port scan, slow POST, slow read) if the feature values "
                    "themselves strongly support that identification — for example, a near-1.0 "
                    "syn_flag_ratio points to a SYN flood, a high fwd_pkts_per_sec with near-zero "
                    "bwd_pkts_per_sec and small pkt_len_mean points to a flood. "
                    "A high anomaly score alone is not sufficient evidence to name an attack type; "
                    "if the pattern is ambiguous, describe the anomalous features without speculating. "
