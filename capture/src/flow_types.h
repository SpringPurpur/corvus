...
typedef struct {
    ...
    // pipeline timing
    // t_enqueue_ns: wall-clock ns when the flow was copied into the IPC ring buffer
    // by ipc_writer_enqueue(). Subtract from Python t_socket_ns to get IPC transfer
    // time (~us). Do NOT subtract first_pkt_ns; that gives flow lifetime, not IPC latency.
    ...
    uint8_t    _pad[1];   // reduced from 9 to 1: 8 bytes moved to t_enqueue_ns above.
