 ...
 *
 * Robin Hood repair on removal: when a slot is freed, any entries in the
 * probe chain that are displaced (i.e., their natural slot is before the
 * freed slot) are shifted back. This keeps average probe length O(1) even
 * under high load rather than degrading to O(n).
 */
...
static inline uint32_t fnv1a_u32(uint32_t hash, uint32_t val)
{
    ...
    hash ^= (val & 0xFF);        hash *= FNV_PRIME_32;
    hash ^= ((val >>  8) & 0xFF); hash *= FNV_PRIME_32;
    hash ^= ((val >> 16) & 0xFF); hash *= FNV_PRIME_32;
    hash ^= ((val >> 24) & 0xFF); hash *= FNV_PRIME_32;
    return hash;
}
 ...
 * was the lower IP (i.e., the original direction is preserved), 0 if swapped.
 */
static int normalise_key(flow_key_t *k)
{
    if (k->src_ip <= k->dst_ip)
        return 1;   // lower IP is already src

    // Swap IPs and ports to put lower IP in src position
                                         ...
                                         int *out_is_new)
{
    ...
    for (uint32_t i = 0; i < FLOW_TABLE_SIZE; i++) {
        if (!t->occupied[slot]) {
            ...
            r->fwd_seg_size_min = UINT32_MAX; // initialise to max so first update sets correct min
            t->occupied[slot] = 1;
            *out_is_new = 1;
            return r;
        }
