...
;   - Caller-saved: rax, rcx, rdx, rsi, rdi, r8, r9, r10, r11, xmm0–xmm7
...
.sum_loop:
    ...
    ; vpmovzxwd: lower 8 × uint16 → 8 × uint32 in ymm2
...
.sq_tail:
    ...
    mov     eax, r13d
    and     eax, 7
    vxorps  xmm3, xmm3, xmm3   ; scalar float accumulator
    movss   xmm4, [r14]         ; mean
...
;   )
;
; Strategy: For each bit position, broadcast a mask byte (e.g. 0x01 for FIN),
...
count_tcp_flags_avx2:
    ...
    mov     rbx, [rsp + 56] ; *ack  (6 pushes × 8 + 8)
    mov     rbp, [rsp + 64] ; *urg  (6 pushes × 8 + 16)
    ...
    mov     eax, 0x20
    vmovd   xmm11, eax
    vpbroadcastb ymm11, xmm11  ; URG mask
